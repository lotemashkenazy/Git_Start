...
def remove_extinct_colors(bubble_colors):
    stack_bubble = Stack.get_stack_bubble()
    for color in bubble_colors:
        count_color = 0
        for i in range(BubblesGrid.get_length()):
            for j in range(consts.BUBBLE_GRID_COLS):
                bubble = BubblesGrid.get_bubble(i, j)
                if color == bubble["color"] or color == stack_bubble["color"]:
                    count_color += 1

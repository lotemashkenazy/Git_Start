...
def find_bubble_location_in_grid(bullet_bubble):
    for row in range(len(bubbles_grid)):
        for col in range(len(bubbles_grid[row])):
            bubble_in_grid = bubbles_grid[row][col]

            if Bubble.should_stop(bubbles_grid, bullet_bubble):
                if bubble_in_grid["color"] == consts.NO_BUBBLE:
                    if Bubble.calc_dist(bullet_bubble, bubble_in_grid) <= consts.BUBBLE_RADIUS * 2:
                        return (row, col)
...
def find_isolated_bubbles():
    ...
    for row in range(len(bubbles_grid)):
        for col in range(len(bubbles_grid[row])):
            bubble_in_grid = bubbles_grid[row][col]
            if bubble_in_grid["color"] != consts.NO_BUBBLE:
                if Bubble.is_isolated(bubbles_grid, (row, col)):
                    isolated_bubbles_list.append((row, col))
    return isolated_bubbles_list


def get_bubble(i, j):
    return bubbles_grid[i][j]
